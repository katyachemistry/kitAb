...
ENV_KITAB="${KITAB_ENV:-kitab}"
ENV_ABB2="${KITAB_ENV_ABB2:-kitab-abb2}"
ENV_ABB3="${KITAB_ENV_ABB3:-kitab-abb3}"
ENV_FLASHABB="${KITAB_ENV_FLASHABB:-kitab-flashabb}"
...
install_abb2() {
    ...
    if ! conda_run_with_env_lib "$ENV_ABB2" python -c "from ImmuneBuilder import ABodyBuilder2; import openmm" 2>/dev/null; then
        warn "ABB2 import check failed (often fixed by: source kitab.local.env — prepends conda lib to LD_LIBRARY_PATH)"
    fi
}
...
install_flashabb() {
    ...
    if ! conda_run_with_env_lib "$ENV_FLASHABB" python -c "from flash_abb import pretrained; import openmm" 2>/dev/null; then
        warn "FlashABB import check failed (often fixed by: source kitab.local.env)"
    fi
}
...
doctor() {
    ...
    check_env "$ENV_KITAB"
    env_exists "$ENV_KITAB" && {
        check_cmd_in_env "$ENV_KITAB" mkdssp
        check_cmd_in_env "$ENV_KITAB" freesasa
