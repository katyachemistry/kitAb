...
#
# Developability-only options (after num_jobs): --average, --hbonds-only, --salt-bridges-only, --aromatic-only,
#   --pka-file <path>, --pH <value>
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --parent-dir)
            ...
            shift 2
            ;;
        --average|--hbonds-only|--salt-bridges-only|--aromatic-only)
            EXTRA_ARGS+=("$1")
            shift
            ;;
        --pka-file)
        ...
        freesasa)
            ...
            if [ "$USE_GNU_PARALLEL" = true ]; then
                ...
                find "$STRUCTURES_DIR" -name "*.pdb" -type f ! -name "*_H.pdb" ! -name "*_L.pdb" | parallel -j "$NUM_JOBS" '
                    ...
                    # Atom-level SASA is stored only for the full complex because that is
                    # all the exposed-atom patch improvement needs.
...
def process_file(pdb_path):
    ...
    # If all required outputs already exist, skip work for this PDB
    # (unless FORCE_SASA=true was set via --force-sasa).
    # Atom-level SASA is stored only for the full complex because that is
    # all the exposed-atom patch improvement needs.
            ...
            if [ "$USE_GNU_PARALLEL" = true ]; then
                process_file() {
                    ...
                    for arg in "${extra_args_array[@]}"; do
                        [ "$arg" = "--pka-file" ] && has_pka_file=true && break
                    done
                    [ "$has_pka_file" = false ] && [ -f "$pka_file" ] && cmd+=("--pka-file" "$pka_file")
                    has_ph=false
                    for arg in "${extra_args_array[@]}"; do
                    ...
                    # Run from SCRIPT_DIR and capture stderr for debugging
                    if ! output=$(cd "$SCRIPT_DIR" && "${cmd[@]}" 2>&1 >/dev/null); then
                        # Print full stderr so the error message is not truncated
                        echo "✗ $basename (failed)"
                        printf '%s\n' "$output"
                    fi
                }
...
def process_file(pdb_path):
    ...
    if '--pka-file' not in extra_args and pka_file.exists():
    ...
    try:
        ...
        if r.returncode != 0:
            # Return full stderr so callers see the real error
            return f"✗ {basename} (failed)\n{r.stderr}"
        return None
    except Exception as e:
